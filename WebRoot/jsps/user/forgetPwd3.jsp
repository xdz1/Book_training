...
<html>
  ...
  <body>
   ...
   <div class="web-width">
    ...
    <form id="f3" action="<c:url value='/ForgetPwdServlet'/>" method="post" class="forget-pwd">
		<input name="method" value="pwd3" type="hidden">
      
       <dl style="margin-top: 60px;">
        <dt>新密码：</dt>
        <dd> <input name="loginpass" id="loginpass"  value="${loginpass }" class="easyui-passwordbox" prompt="登录密码"   
        ...
        <dd class="error"> <label> ${errors.loginpass}</label></dd>
        <div class="clears"></div>
       </dl> 
       <dl>
        <dt>确认密码：</dt>
        <dd><input name="reloginpass" id="reloginpass" value="${reloginpass }" class="easyui-passwordbox" prompt="确认密码" 
        data-options="required: true, validType:'repwdFormat',validateOnCreate:false,validateOnBlur:true"
        iconWidth="28" style="width:48%;height:34px;padding:10px;"></dd>
         <dd class="error"> <label> ${errors.reloginpass}</label></dd>
        <div class="clears"></div>
       </dl> 
      <div class="subtijiao"><input type="image" onclick="submitForm3()" id="submit" src="<c:url value='/images/next.png'/>" class="loginBtn"/></div> 
      </form><!--forget-pwd/-->
   </div><!--web-width/-->
  </div><!--content/-->
